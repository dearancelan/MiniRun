...
const JobBiz = require('../../../../biz/follow_biz.js');
...
Page({
	...
	_setSort: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/follow_sort', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'FOLLOW_ORDER', sort);
				this.setData({
	...
	_del: async function (e) {
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('admin/follow_del', params, opts).then(res => {
	...
	bindMoreTap: async function (e) {
		...
		let order = this.data.dataList.list[idx].FOLLOW_ORDER;
			...
			success: async res => {
				switch (res.tapIndex) {
					case 0: { //预览
						let id = pageHelper.dataset(e, 'id');
						wx.navigateTo({
							url: '../../../follow/detail/follow_detail?id=' + id,
						});
						break;
					}
					case 1: { //置顶 
	...
	_setStatus: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/follow_status', params).then(res => {
	...
	_getSearchMenu: function () {
		...
		let sortMenus = [
			...
			{ label: '最早ˇ', type: 'sort', value: 'FOLLOW_ADD_TIME|asc' },
			{ label: '浏览数ˇ', type: 'sort', value: 'FOLLOW_VIEW_CNT|desc' },
			{ label: '收藏数ˇ', type: 'sort', value: 'FOLLOW_FAV_CNT|desc' },
